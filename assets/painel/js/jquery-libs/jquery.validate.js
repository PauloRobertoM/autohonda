...
$.extend($.fn, {
	...
	// if nothing is selected, return nothing; can't chain anyway
		if (!this.length) {
			options && options.debug && window.console && console.warn( "nada selecionado, impossivel validar, nada retornando" );
			return;
		}
...
messages: {
		...
		creditcard: "Digite o número do cartão de crédito válido.",
		equalTo: "Digite aqui a mesma coisa.",
		accept: "Digite uma extensão válida.",
		maxlength: $.validator.format("Digite o máximo de {0} caracteres."),
		minlength: $.validator.format("Digite o mínimo de {0} caracteres."),
 ...
 * NOVO METODO PARA O JQUERY VALIDATE
 * VALIDA CPF 
 * A VALIDAÇÃO É FEITA COM OU SEM OS CARACTERES SEPARADORES, PONTO, HIFEN
 *
 * ESTE MÉTODO FOI ADAPTADO POR:
 * 
 * Shiguenori Suguiura Junior <junior@dothcom.net>
...
jQuery.validator.addMethod("yearCount", function(value, element) {
   var x = ($('#ano_validade_cartao').val() == 4) && ($('#mes_validade_cartao').val() == 0),
   y = ($('#ano_validade_cartao').val() < 4),
   z = (x || y);
   return z;
}, "Ano inválido.");
