...
class Noticias extends MY_Controller
{
	...
	public function add_upd($id = NULL, $pagina = NULL)
	{
		...
		$model_albuns			=	load_model('generico', 'albuns');
		$array_albuns['where']	=	array(
				'status' => 1
		);
		// application/models/my_model.php
		$query_albuns			=	$model_albuns->get_query($array_albuns, NULL, NULL, FALSE);
		// application/models/model_generico.php
		$data['albuns']			=	$model_albuns->render_query(array(
				'query'	=>	$query_albuns,
				'type'	=>	'array'
			)
		);
		...
		$model_modulos				=	load_model('generico', 'modulos');
		$array_modulos['where']		=	array(
				"file"		=>	1,
				"status"	=>	1
		);
		// application/models/my_model.php
		$query_modulos				=	$model_modulos->get_query($array_modulos, NULL, NULL, FALSE);
		// application/models/model_generico.php
		$data['modulos']			=	$model_modulos->render_query(array(
				'query'	=>	$query_modulos,
				'type'	=>	'array'
			)
		);
		...
		if($this->form_validation->run() == TRUE)
		{
			if(!is_null($pagination) && post('pagination') == $pagination)
			{
				...
				if(post("id") != "")
				{
					...
					$data_update = array(
							'data'			=>	format_data_db(post("data").' '.post("time")),
